...
class Address extends \Simi\Simiconnector\Helper\Data
{
    ...
    public function getAddressDetail($data, $customer = null)
    {
        ...
        $detail = [
            ...
            'country_name' => $data->getCountryId() ?
                $this->simiObjectManager->create('\Magento\Directory\Model\CountryFactory')->create()
                ->loadByCode($data->getCountryId())->getName() : null,
            ...
            'area'          => $data->getArea(),
            'area_name'          => $this->simiObjectManager->get('Simi\Simiaddress\Helper\Address')->getAreaNameByCode($data->getArea()),
            'block'          => $data->getBlock(),
            'avenue'          => $data->getAvenue(),
            'building_no'          => $data->getBuildingNo(),
            'floor'          => $data->getFloor(),
            'apartment'          => $data->getApartment(),
            'delivery_instruction'          => $data->getDeliveryInstruction(),
            'location_name'          => $data->getLocationName()
        ];
        if ($customer != null) {
    ...
    public function saveBillingAddress($billingAddress)
    {
        ...
        if (isset($billingAddress->entity_id) && $billingAddress->entity_id) {
            ...
            $billingAddress->setArea($addressInterface->getArea());
            $billingAddress->setBlock($addressInterface->getBlock());
            $billingAddress->setAvenue($addressInterface->getAvenue());
            $billingAddress->setBuildingNo($addressInterface->getBuildingNo());
            $billingAddress->setFloor($addressInterface->getFloor());
            $billingAddress->setApartment($addressInterface->getApartment());
            $billingAddress->setDeliveryInstruction($addressInterface->getDeliveryInstruction());
            $billingAddress->setLocationName($addressInterface->getLocationName());
    ...
    public function saveShippingAddress($shippingAddress)
    {
        ...
        if (!isset($shippingAddress->entity_id)) {
            ...
            $addressModel = $this->simiObjectManager->create('Magento\Customer\Model\Address')->load($shippingAddress->entity_id);
