...
class Homeproductlists extends Apiabstract
{
    ...
    private function _addInfo($dataArray)
    {
        ...
        $fields = array(
            ...
            'description','short_description','is_salable','url_key,url_path','origin', 'unit'
        );
        if ($this->SHOW_PRODUCT_ARRAY) {
            $productCollection                          = $listHelper->getProductCollection($listModel);
            $productListAPIModel = $this->simiObjectManager->get('Simi\Simicustomize\Model\Api\Products');
