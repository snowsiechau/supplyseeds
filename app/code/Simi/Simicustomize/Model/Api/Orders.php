...
class Orders extends Apiabstract
{
    ...
    private function _updateOrder()
    {
        ...
        if (isset($parameters['b_address'])) {
            $this->_initCheckout();
            $this->simiObjectManager->get('Simi\Simicustomize\Helper\Address')
                    ->saveBillingAddress($parameters['b_address']);
            if (!isset($parameters['s_address']) && (!$this->_getQuote()->getShippingAddress()->getFirstName())) {
                $parameters['s_address'] = $parameters['b_address'];
            }
    ...
    public function store()
    {
        ...
        if (!$this->simiObjectManager->create('Magento\Customer\Model\Session')->isLoggedIn() &&
            ...
            // $customerObj = (object) $billingAddress->getData();
            // $customerObj->simi_phone = $customerObj->telephone;
            // $customer = $this->simiObjectManager->create('Simi\Simicustomize\Model\Customer')->_createCustomer($customerObj);

            // $addressDataArray = $this->simiObjectManager->get('Simi\Simicustomize\Helper\Address')
            //         ->getAddressDetail($billingAddress, $customer);
            // $address   = $this->simiObjectManager->create('Magento\Customer\Model\Address');
            // $address->setData($addressDataArray);
            // $address->setId(null);
            ...
            // $addressForm->compactData($addressDataArray);
            ...
            // $customer= $this->simiObjectManager->create('\Magento\Customer\Api\CustomerRepositoryInterface')->getById($customer->getId());
            // $this->_getQuote()->assignCustomer($customer);
            ...
            $customer->setCustomAttribute('simi_phone', $billingAddress->getData('telephone'));
            $addressDataArray = $this->simiObjectManager->get('Simi\Simicustomize\Helper\Address')
                    ->getAddressDetail($billingAddress, $customer);
    ...
    public function show()
    {
        ...
        if ($data['resourceid'] == 'onepage') {
            ...
            */
            ...
            $order['billing_address']  = $this->simiObjectManager->get('Simi\Simicustomize\Helper\Address')
                    ->getAddressDetail($quote->getBillingAddress(), $customer);
            $order['shipping_address'] = $this->simiObjectManager->get('Simi\Simicustomize\Helper\Address')
    ...
    private function _updateOrderInformation(&$order, $customer)
    {
        ...
        $dateUpdate = new \DateTime($order['updated_at']);
        ...
        $order['created_at']  = $date->setTimezone(new \DateTimeZone('Asia/Kuwait'))->format('Y-m-d H:i:s');
        $order['updated_at']  = $dateUpdate->setTimezone(new \DateTimeZone('Asia/Kuwait'))->format('Y-m-d H:i:s');
