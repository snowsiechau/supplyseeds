...
class Products extends \Simi\Simiconnector\Model\Api\Apiabstract
{
    ...
    public function setBuilderQuery()
    {
        ...
        if ($data['resourceid']) {
            ...
            if (isset($parameters[self::FILTER])) {
                ...
                } elseif (isset($filter['best_seller'])) {
                    $this->setFilterByBestSeller($filter['best_seller']);
                } else {
    ...
    public function index()
    {
        ...
        foreach ($collection as $entity) {
            ...
            if ($entity->getResource()->getAttribute('origin')->getFrontend()->getValue($entity)) {
                $info_detail['origin'] = $entity->getResource()->getAttribute('origin')->getFrontend()->getValue($entity);
            }
            if ($entity->getResource()->getAttribute('unit_name')->getFrontend()->getValue($entity)) {
                $info_detail['unit_name'] = $entity->getResource()->getAttribute('unit_name')->getFrontend()->getValue($entity);
            }
            if ($entity->getResource()->getAttribute('unit_number')->getFrontend()->getValue($entity)) {
                $info_detail['unit_number'] = $entity->getResource()->getAttribute('unit_number')->getFrontend()->getValue($entity);
            }
    ...
    public function show()
    {
        ...
        if ($entity->getResource()->getAttribute('origin')->getFrontend()->getValue($entity)) {
            $info['origin'] = $entity->getResource()->getAttribute('origin')->getFrontend()->getValue($entity);
        }
        if ($entity->getResource()->getAttribute('unit_name')->getFrontend()->getValue($entity)) {
            $info['unit_name'] = $entity->getResource()->getAttribute('unit_name')->getFrontend()->getValue($entity);
        }
        if ($entity->getResource()->getAttribute('unit_number')->getFrontend()->getValue($entity)) {
            $info['unit_number'] = $entity->getResource()->getAttribute('unit_number')->getFrontend()->getValue($entity);
        }
    ...
    public function setFilterByBestSeller(){
        $data = $this->getData();
        $categoryID = $data['params']['filter']['best_seller'];
        $categoryModel = $this->simiObjectManager->create('\Magento\Catalog\Model\Category')->load($categoryID);
        $listProduct = '';
        $listProduct =  str_replace(' ', '', $categoryModel->getBestSellerProduct());
        $this->builderQuery = $this->simiObjectManager
            ->create('Simi\Simiconnector\Model\ResourceModel\Productlist\ProductlistCollection')
            ->getProductCollectionByType(1, $this->simiObjectManager, $listProduct);
